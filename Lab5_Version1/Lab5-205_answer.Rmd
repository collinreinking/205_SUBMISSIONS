...
The schema for the customer table is **public**.
```
...
All three plans show that they are scanning (in sequence) which ever tables whose names start with the string that I gave after FROM.  When multiple tables start with that string it scans them one at a time, appending the results to Result.
...
For the Union query the width from each table scanned is 28, which reduces to 13 only after the final hash aggregate
